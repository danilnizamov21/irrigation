...
async def main():
    ...
    if days is not None:
        for i in range(len(days)):
            ...
            calc =  Analys.calculate(t_max=day.max_temperature,
                                     t_min=day.min_temperature,
                                     t_avg=day.avg_temperature,
                                     rh=day.avg_humidity,
                                     ghi=day.total_radiation)
            a = Analys()
            analys  = await a.classifier_data_fetcher(calc,10 , day.max_precipitation_probability, day.total_precipitation)
